// from "custom-calendar-taro": "1.0.12"
...
class CustomCalendar extends Component<IProps, IState> {
  ...
  getPickerText = () => {
    ...
    if (view === "week") {
      ...
      const startMonthStr = formatDate(new Date(currentDateObj.getTime() - 1000 * 60 * 60 * 24 * 6), "month");

      if (startMonthStr !== monthStr) {
        return `${startMonthStr}  ~  ${monthStr}`;
      }
      return monthStr
    }
    if (view === "month") {
  ...
  switchView = () => {
    this.setState(preState => ({ view: preState.view === 'month' ? 'week' : 'month' }))
  }
  ...
  goToday = () => {
    ...
    this.props.onDayClick?.({ value: today })
  }

  goNext = () => {
    const { currentCarouselIndex, current, view } = this.state;
  ...
  render() {
    const { current, selectedDate, currentCarouselIndex, selectedRange, view } =
      this.state;
    let {
      marks,
      isVertical,
    ...
    return (
      <View className='custom-calendar'>
        {!hideController && (
          ...
          >
            <View
              className='calendar-picker-switch'
            ...
            >
              <View className='calendar-picker-text'>
                {pickerTextGenerator
        ...
        {isSwiper ? (
          <Swiper
            style={{
              height: view === "month" ? "27rem" : "4.5rem",
              ...bodyStyle,
            }}
            vertical={isVertical}
            circular
